...
function TimelineItem({ isLast, icon, title, date, description, bgColor = "bg-blue-600" }) {
    return (
        <div className="relative flex items-start">
            ...
            <div className={`h-8 w-8 rounded-full ${bgColor} flex items-center justify-center relative z-10`}>
                {icon}
            </div>
...
function CargoDetails({ cargoDetails }) {
    ...
    return (
        <div className="mt-4 sm:mt-6 border border-gray-200 rounded-lg p-3 sm:p-4">
            <h3 className="text-xs sm:text-sm font-medium text-gray-900 mb-3 sm:mb-4">Cargo Details</h3>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4">
                <div>
                    <h4 className="text-xs font-medium text-gray-500">Weight</h4>
                    <p className="mt-1 text-xs sm:text-sm text-gray-900">{cargoDetails.weight} kg</p>
                </div>
                <div>
                    <h4 className="text-xs font-medium text-gray-500">Type</h4>
                    <p className="mt-1 text-xs sm:text-sm text-gray-900 capitalize">{cargoDetails.type}</p>
                </div>
                <div>
                    <h4 className="text-xs font-medium text-gray-500">Dimensions</h4>
                    <p className="mt-1 text-xs sm:text-sm text-gray-900">{cargoDetails.dimensions}</p>
                </div>
            </div>
        </div>
    );
...
function StatusTimeline({ shipment }) {
    ...
    return (
        <div className="mt-6">
            <h3 className="text-lg font-medium text-gray-900 mb-6">Status Timeline</h3>
            <div className="flow-root">
                <ul className="-mb-8">
                    {/* Creation (always shown first) */}
                    <li>
                        <TimelineItem 
                            isLast={!shipment.tracking?.updates?.length && !shipment.tracking?.currentLocation}
                            icon={checkmarkIcon}
                    ...
                    {shipment.tracking?.updates?.map((update, index) => (
                        <li key={update._id}>
                            <TimelineItem 
                                isLast={!shipment.tracking?.currentLocation && index === shipment.tracking.updates.length - 1}
                                icon={transitIcon}
                                title={update.note}
    ...
    return (
        <div className="mt-4 sm:mt-6 flex flex-col sm:flex-row sm:justify-between space-y-3 sm:space-y-0">
            ...
            <div className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-3">
                {shipment.status !== 'cancelled' && shipment.status !== 'delivered' && shipment.status !== 'in-transit' &&(
                    <button
